...
export default function App(){
 ...
 useEffect(()=>{if(!mapEl.current||mapRef.current)return;const map=new maplibregl.Map({container:mapEl.current,style:"https://tiles.openfreemap.org/styles/positron",center:[-71.305,41.498],zoom:12});map.addControl(new maplibregl.NavigationControl(),"bottom-right");map.on("load",()=>setMapReady(x=>x+1));map.on("contextmenu",(ev:MapMouseEvent)=>{setPoint({lat:ev.lngLat.lat,lon:ev.lngLat.lng});setSelectedTaskId(null);setTaskType("opengrid.tasks.v1.Navigate");setRadius(20);setReview(false);setComposer(true);});mapRef.current=map;},[]);
