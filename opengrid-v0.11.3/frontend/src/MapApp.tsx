 ...
 useEffect(()=>{
  ...
  for(const a of overlays){
   ...
   if(meta.overlay_type==="WMS"&&meta.wms_url){
    const q=new URLSearchParams({service:"WMS",request:"GetMap",version:"1.1.1",layers:String(meta.layers||"0"),styles:"",format:String(meta.format||"image/png"),transparent:"true",srs:"EPSG:3857",width:"256",height:"256",bbox:"{bbox-epsg-3857}"});
    url=`${meta.wms_url}?${q.toString()}`.replace(encodeURIComponent("{bbox-epsg-3857}"),"{bbox-epsg-3857}");
   }
   if(!url)continue;
   const setting=settings[a.artifact_id]||{};
   const opacity=setting.enabled?Number(setting.opacity??meta.default_opacity??0.72):0;
   if(!map.getSource(id))map.addSource(id,{type:"raster",tiles:[url],tileSize:256});
   if(!map.getLayer(id))map.addLayer({id,type:"raster",source:id,paint:{"raster-opacity":opacity}},map.getStyle().layers?.find((x:any)=>x.type==="symbol")?.id);
