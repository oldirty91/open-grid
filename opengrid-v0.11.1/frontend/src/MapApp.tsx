...
export default function App(){
 ...
 useEffect(()=>{
  ...
  for(const a of overlays){
   const id=`overlay-${a.artifact_id}`,url=a.metadata?.tile_url;if(!url)continue;
   const setting=settings[a.artifact_id]||{};
   const opacity=setting.enabled?Number(setting.opacity??a.metadata?.default_opacity??0.72):0;
