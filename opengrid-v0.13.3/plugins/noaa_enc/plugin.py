...
async def discover_local_layers(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for path in geojson_files:
        try:
            ...
            if existing_mtime != source_mtime_ns or not feature_exists:
...
async def discover_layers():
    async with _discovery_lock:
        ...
        visible = [{
            ...
            "description": f"Local NOAA ENC feature class from {x['dataset']}",
            ...
            "min_zoom": x["min_zoom"], "max_zoom": x["max_zoom"],
            "style": x["style"], "metadata": x["metadata"],
        } for x in discovered if x["class_name"] not in disabled_classes]
        await api("PUT", f"/api/v1/plugins/{PLUGIN_ID}/gis-layers", json={"layers": visible})
