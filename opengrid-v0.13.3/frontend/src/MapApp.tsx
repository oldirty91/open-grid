...
export default function App(){
 ...
 useEffect(()=>{
  ...
  const refresh=async()=>{
   ...
   if(!canceled&&sequence===gisRefreshSequenceRef.current){setEncLoading(false);setEncStatus(`${loaded}/${visibleAtZoom.length} ENC layers ready`);setGisCacheVersion(v=>v+1)}
  };
 ...
 const pluginGeoEntities=useMemo(()=>{const q=filter.toLowerCase();const rows:any[]=[];for(const [sourceId,cache] of Object.entries(gisFeatureCacheRef.current)){for(const feature of cache.values()){const p=feature.properties||{};const name=String(p.OBJNAM||p.NOBJNM||p.__og_class_name||p.__og_layer_name||"Geo feature");const hay=`${name} ${p.__og_class_name||""} ${p.__og_dataset||""}`.toLowerCase();if(q&&!hay.includes(q))continue;rows.push({...feature,__sourceId:sourceId,__displayName:name});if(rows.length>=1500)return rows}}return rows},[filter,gisCacheVersion]);
...
useEffect(()=>{
 ...
 const coreGeoFeatures=entities.filter(e=>e.template==="GEOENTITY").flatMap(e=>{const raw=e.components?.geometry?.geojson||e.components?.geometry||e.components?.geojson;if(!raw?.type)return[];return[{type:"Feature",properties:{entity_id:e.entity_id,name:nameOf(e),style:e.components?.style||{}},geometry:raw}]});
 ...
 if(!map.getLayer("core-geoentities-fill"))map.addLayer({id:"core-geoentities-fill",type:"fill",source:"core-geoentities",filter:["==",["geometry-type"],"Polygon"],paint:{"fill-color":"#8ecae6","fill-opacity":0.18,"fill-outline-color":"#8ecae6"}});
 if(!map.getLayer("core-geoentities-line"))map.addLayer({id:"core-geoentities-line",type:"line",source:"core-geoentities",filter:["in",["geometry-type"],["literal",["LineString","Polygon"]]],paint:{"line-color":"#8ecae6","line-width":2}});
 if(!map.getLayer("core-geoentities-point"))map.addLayer({id:"core-geoentities-point",type:"circle",source:"core-geoentities",filter:["==",["geometry-type"],"Point"],paint:{"circle-color":"#8ecae6","circle-radius":6,"circle-stroke-color":"#071018","circle-stroke-width":1}});
...
useEffect(()=>{
 ...
 const click=(ev:any)=>{
  ...
  if(gisIds.length){
   ...
   if(gis){setSelectedGisFeature({properties:gis.properties||{},geometry:gis.geometry});setSelectedEntity(null);setSelectedTaskId(null);return}
  }
  const coreGeoLayers=["core-geoentities-fill","core-geoentities-line","core-geoentities-point"].filter(id=>Boolean(map.getLayer(id)));
  const coreGeo=coreGeoLayers.length?map.queryRenderedFeatures(ev.point,{layers:coreGeoLayers})?.[0]:null;
  if(coreGeo){const entity=entities.find(e=>e.entity_id===coreGeo.properties?.entity_id);if(entity){setSelectedGisFeature(null);setSelectedEntity(entity);setSelectedTaskId(null);setRightOpen(true);return}}
  ...
  if(!entity)return;
  setSelectedGisFeature(null);setSelectedEntity(entity);setSelectedTaskId(null);
 ...
 const move=(ev:any)=>{
  ...
  const layers=["entity-icons","core-geoentities-fill","core-geoentities-line","core-geoentities-point",...gisIds].filter(id=>Boolean(map.getLayer(id)));
