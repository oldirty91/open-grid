...
async def health():
    return {"status": "ok", "service": "opengrid-api", "version": "0.13.1", "artifact_storage_ready": bool(getattr(app.state, "artifact_storage_ready", False))}
...
@app.get("/api/v1/maps", tags=["Artifacts"])
async def list_offline_maps():
    import httpx
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            response=await client.get(f"{settings.map_engine_url.rstrip('/')}/maps");response.raise_for_status();return response.json()
    except Exception as exc:
        raise HTTPException(502,f"Map Engine unavailable: {exc}")

@app.post("/api/v1/maps", tags=["Artifacts"])
async def upload_offline_map(file: UploadFile = File(...)):
    import httpx
    data=await file.read()
    try:
        async with httpx.AsyncClient(timeout=300) as client:
            response=await client.post(f"{settings.map_engine_url.rstrip('/')}/maps",files={"file":(file.filename,data,file.content_type or "application/octet-stream")});response.raise_for_status();return response.json()
    except httpx.HTTPStatusError as exc:
        raise HTTPException(exc.response.status_code,exc.response.text)
    except Exception as exc:
        raise HTTPException(502,f"Map Engine unavailable: {exc}")

@app.delete("/api/v1/maps/{layer_id}", tags=["Artifacts"])
async def delete_offline_map(layer_id: str):
    import httpx
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            response=await client.delete(f"{settings.map_engine_url.rstrip('/')}/maps/{layer_id}");response.raise_for_status();return response.json()
    except httpx.HTTPStatusError as exc:
        raise HTTPException(exc.response.status_code,exc.response.text)
    except Exception as exc:
        raise HTTPException(502,f"Map Engine unavailable: {exc}")
