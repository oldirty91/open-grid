...
export default function App(){
 ...
 const load=()=>fetch(`${API}/api/v1/entities/${selectedEntity.entity_id}/location-history?limit=1000`)
  .then(r=>r.ok?r.json():[])
...
useEffect(()=>{
 ...
 setSource("breadcrumb-points",{type:"FeatureCollection",features:crumbs});
 ...
 if(!map.getLayer("breadcrumb-points"))map.addLayer({
  id:"breadcrumb-points",type:"circle",source:"breadcrumb-points",
  paint:{"circle-radius":4,"circle-color":"#00d8ff","circle-stroke-color":"#071018","circle-stroke-width":1}
 });
...
useEffect(()=>{
 const map=mapRef.current;
 if(!map||!map.isStyleLoaded()||!map.getLayer("entity-icons"))return;
 const click=(ev:any)=>{
  const id=ev.features?.[0]?.properties?.entity_id;
 ...
 map.on("click","entity-icons",click);
 map.on("mouseenter","entity-icons",enter);
 map.on("mouseleave","entity-icons",leave);
 return()=>{
  map.off("click","entity-icons",click);
  map.off("mouseenter","entity-icons",enter);
  map.off("mouseleave","entity-icons",leave);
 };
},[entities,mapReady]);
