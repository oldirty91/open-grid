...
app=FastAPI(title="OpenGrid Map Engine",version="0.13.0")
...
def inspect(path:Path)->dict[str,Any]:
    ...
    return {"layer_id":layer_id,"name":meta.get("name",path.stem),"filename":path.name,"path":str(path),"format":fmt,
...
async def register():
    await api("POST","/api/v1/plugins/register",json={"plugin_id":PLUGIN_ID,"name":"Map Engine","version":"0.13.0","plugin_type":"GIS_DATA_SOURCE","protocol":"MBTiles","capabilities":["gis.layer_provider","gis.offline","gis.mbtiles","gis.tile_server"],"configuration_schema":{"type":"object","properties":{}},"configuration":{}})
    layers=[]
...
async def rescan():
    global _catalog
    fresh={}
    for path in sorted(LIBRARY.glob("*.mbtiles")):
        try:
