...
async def discover_layers():
    ...
    fresh_catalog: dict[str, dict[str, Any]] = {}

    for dataset, (service_url, min_zoom, max_zoom) in services.items():
    ...
    await api("POST", "/api/v1/plugins/register", json={
        "plugin_id": PLUGIN_ID, "name": "NOAA ENC", "version": "0.12.4",
...
app = FastAPI(title="OpenGrid NOAA ENC Plugin", version="0.12.4", lifespan=lifespan)
