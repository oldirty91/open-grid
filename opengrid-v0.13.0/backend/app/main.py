...
async def health():
    return {"status": "ok", "service": "opengrid-api", "version": "0.13.0", "artifact_storage_ready": bool(getattr(app.state, "artifact_storage_ready", False))}
...
async def location_history(
    ...
    result = await db.execute(text("""
        WITH revision_locations AS (
            ...
            FROM entity_revisions
            WHERE entity_id = :entity_id
              AND received_time >= NOW() - (:hours * INTERVAL '1 hour')
              AND (
