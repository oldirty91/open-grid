...
export default function App(){
 ...
 useEffect(()=>{Promise.all([fetch(`${API}/api/v1/entities`),fetch(`${API}/api/v1/tasks`),fetch(`${API}/api/v1/task-definitions`),fetch(`${API}/api/v1/artifacts`),fetch(`${API}/api/v1/gis/layers`)]).then(async([e,t,d,a,g])=>{if(e.ok){const items:Entity[]=await e.json();setEntities(items);setHistory(p=>items.reduce((acc,item)=>appendLiveSample(acc,item),p));Promise.all(items.map(item=>fetch(`${API}/api/v1/entities/${encodeURIComponent(item.entity_id)}/location-history?limit=20000&hours=24`).then(r=>r.ok?r.json():[]).then((samples:Sample[])=>({id:item.entity_id,samples})).catch(()=>({id:item.entity_id,samples:[]})))).then(all=>setHistory(p=>all.reduce((acc,x)=>({...acc,[x.id]:mergeSamples(x.samples,acc[x.id]||[])}),p)));}if(t.ok)setTasks(await t.json());if(d.ok)setDefs(await d.json());if(a.ok)setOverlays((await a.json()).filter((x:any)=>x.artifact_type==="MAP_OVERLAY"));if(g.ok)setGisLayers(await g.json());});},[]);
 ...
 useEffect(()=>{
  ...
  const refresh=async()=>{
   ...
   if(!canceled&&sequence===gisRefreshSequenceRef.current){setEncLoading(false);setEncStatus(`${loaded}/${visibleAtZoom.length} ENC layers ready`)}
  };
 ...
 const load=()=>fetch(`${API}/api/v1/entities/${selectedEntity.entity_id}/location-history?limit=20000&hours=24`)
  .then(r=>r.ok?r.json():[])
