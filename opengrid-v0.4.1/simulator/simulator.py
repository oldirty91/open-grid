        ...
        json={
            ...
            "components": {
                ...
                "ontology": {
                    ...
                    "domain": "MARITIME",
                    "track_type": "SOURCE",
                    "specific_type": ("CARGO_SHIP" if source.startswith("ais") else "SURFACE_CONTACT"),
                },
                **({"ais": {"ship_type": "CARGO_SHIP"}} if source.startswith("ais") else {}),
                "location": {
                    "latitude": lat,
                    "longitude": lon,
                    "heading_degrees": 64,
...
async def read_task(client, task_id):
    response = await client.get(f"{API}/api/v1/tasks")
    response.raise_for_status()
    return next((task for task in response.json() if task["task_id"] == task_id), None)
        ...
        while True:
            ...
            for asset_id, asset in ASSETS.items():
                ...
                if asset_id in ACTIVE:
                    ...
                    task = active["task"]
                    latest = await read_task(client, task["task_id"])
                    if latest and latest["status"] == "STATUS_CANCELED":
                        asset["speed"] = 0
                        del ACTIVE[asset_id]
                        await patch(client, asset_id, "location", {
                            "latitude": asset["lat"], "longitude": asset["lon"],
                            "heading_degrees": asset["heading"], "speed_mps": 0,
                        })
                        continue
