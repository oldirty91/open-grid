...
async def health():
    return {"status": "ok", "service": "opengrid-api", "version": "0.4.1"}
...
@app.get(
    ...
    tags=["Entities"],
    summary="Read durable entity location history",
    description=(
        "Returns persisted latitude/longitude samples from the database-backed "
        "location-history projection. History survives browser and service restarts."
    ),
)
async def location_history(
    ...
    result = await db.execute(text("""
        SELECT revision, latitude, longitude, heading_degrees,
               speed_mps, observed_at
...
async def list_plugins(db: AsyncSession = Depends(get_db)):
    result=await db.execute(text("SELECT * FROM plugins ORDER BY name"));return [dict(x) for x in result.mappings()]
