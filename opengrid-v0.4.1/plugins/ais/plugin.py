...
async def main():
 async with httpx.AsyncClient(timeout=15) as c:
  ...
  while True:
   try:
    await register(c)
    await call(c,"POST",f"/api/v1/plugins/{PLUGIN_ID}/logs",json={"level":"INFO","message":"AIS plugin registered","details":{"mode":MODE}})
    break
   except Exception as e:
    print("register",e)
    await asyncio.sleep(2)
  asyncio.create_task(heartbeat(c));source=replay() if MODE=='replay' else udp() if MODE=='udp' else tcp()
