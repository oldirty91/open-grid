...
CREATE TABLE IF NOT EXISTS plugin_gis_layers (
    ...
    max_zoom DOUBLE PRECISION NOT NULL DEFAULT 24,
    style JSONB NOT NULL DEFAULT '{}',
    metadata JSONB NOT NULL DEFAULT '{}',
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    PRIMARY KEY(plugin_id, layer_id)
);
...
ALTER TABLE entities_current ADD COLUMN IF NOT EXISTS component_times JSONB NOT NULL DEFAULT '{}';
"""
...
async def seed_installed_plugins() -> None:
    ...
    async with engine.begin() as conn:
        await conn.execute(text("DELETE FROM plugins WHERE plugin_id = 'opengrid.camera.ridot'"))
        # Remove registry residue from the retired MBTiles/Map Engine experiment.
        await conn.execute(text("DELETE FROM plugin_gis_layers WHERE plugin_id IN ('opengrid.map_engine','opengrid.mbtiles','opengrid.map-engine')"))
        await conn.execute(text("DELETE FROM plugin_logs WHERE plugin_id IN ('opengrid.map_engine','opengrid.mbtiles','opengrid.map-engine')"))
        await conn.execute(text("DELETE FROM plugins WHERE plugin_id IN ('opengrid.map_engine','opengrid.mbtiles','opengrid.map-engine')"))
