...
useEffect(()=>{
 ...
 const coreGeoFeatures=entities.filter(e=>e.template==="GEOENTITY").flatMap(e=>{const raw=e.components?.geometry?.geojson||e.components?.geometry||e.components?.geojson;if(!raw?.type)return[];const s=e.components?.style||{};return[{type:"Feature",properties:{entity_id:e.entity_id,name:nameOf(e),class_name:e.components?.ontology?.specific_type||"GEOENTITY",fillColor:s.fillColor,lineColor:s.lineColor,circleColor:s.circleColor,fillOpacity:s.fillOpacity,lineWidth:s.lineWidth,circleRadius:s.circleRadius},geometry:raw}]});
 ...
 if(!map.getLayer("core-geoentities-point"))map.addLayer({id:"core-geoentities-point",type:"circle",source:"core-geoentities",filter:["==",["geometry-type"],"Point"],paint:{"circle-color":["coalesce",["get","circleColor"],"#8ecae6"],"circle-radius":["coalesce",["get","circleRadius"],6],"circle-stroke-color":"#071018","circle-stroke-width":1}});
