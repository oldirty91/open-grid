...
export default function App(){
 ...
 useEffect(()=>{Promise.all([fetch(`${API}/api/v1/entities`),fetch(`${API}/api/v1/tasks`),fetch(`${API}/api/v1/task-definitions`),fetch(`${API}/api/v1/artifacts`)]).then(async([e,t,d,a])=>{if(e.ok){const items:Entity[]=await e.json();setEntities(items);setHistory(p=>items.reduce((acc,item)=>appendLiveSample(acc,item),p));}if(t.ok)setTasks(await t.json());if(d.ok)setDefs(await d.json());if(a.ok)setOverlays((await a.json()).filter((x:any)=>x.artifact_type==="MAP_OVERLAY"));});},[]);
 ...
 useEffect(()=>{const map=mapRef.current;if(!map||!mapReady)return;for(const a of overlays){const id=`overlay-${a.artifact_id}`;const url=a.metadata?.tile_url;if(!url)continue;if(!map.getSource(id))map.addSource(id,{type:"raster",tiles:[url],tileSize:256});if(!map.getLayer(id))map.addLayer({id,type:"raster",source:id,paint:{"raster-opacity":overlayOn?overlayOpacity:0}},map.getStyle().layers?.find((x:any)=>x.type==="symbol")?.id);else map.setPaintProperty(id,"raster-opacity",overlayOn?overlayOpacity:0)}} ,[overlays,overlayOn,overlayOpacity,mapReady]);
