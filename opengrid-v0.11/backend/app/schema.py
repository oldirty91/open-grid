...
async def seed_installed_plugins() -> None:
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(text("""
            INSERT INTO plugins (plugin_id,name,version,plugin_type,protocol,capabilities,configuration_schema,configuration,enabled,status,status_message,metrics)
            VALUES ('opengrid.camera.ridot','RIDOT Fixed Camera','0.1.0','SENSOR_ADAPTER','HTTP_IMAGE',
              '["entity.publish","task.consume","task.status","artifact.publish"]'::jsonb,
              '{"type":"object"}'::jsonb,'{"mode":"demo"}'::jsonb,TRUE,'OFFLINE','Awaiting plugin heartbeat','{}'::jsonb)
            ON CONFLICT (plugin_id) DO NOTHING
        """))
