...
async def seed_demo_overlay():
    async with __import__("app.db", fromlist=["engine"]).engine.begin() as conn:
        await conn.execute(text("""
          INSERT INTO artifacts(name,artifact_type,content_type,object_name,size_bytes,sha256,metadata)
          VALUES ('NOAA ENC — Block Island Sound','MAP_OVERLAY','application/vnd.opengrid.map-overlay+json',
                  'system/noaa-enc-block-island-sound.json',0,'system',
                  '{"overlay_type":"RASTER_TILES","tile_url":"https://gis.charttools.noaa.gov/arcgis/rest/services/MarineChart_Services/NOAACharts/MapServer/WMTS/tile/1.0.0/MarineChart_Services_NOAACharts/default/GoogleMapsCompatible/{z}/{y}/{x}.png","bounds":[-72.15,40.85,-70.75,41.75],"default_opacity":0.72,"source":"NOAA Chart Display Service","navigation_warning":"For situational awareness only; not certified for navigation"}'::jsonb)
...
async def health():
    return {"status": "ok", "service": "opengrid-api", "version": "0.11.0", "artifact_storage_ready": bool(getattr(app.state, "artifact_storage_ready", False))}
