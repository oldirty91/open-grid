...
function soundingValue(e:Entity){const a=e.components?.attributes||{};const raw=a.VALSOU??a.DRVAL1??a.DRVAL2??a.value??a.VALUE;const value=Number(Array.isArray(raw)?raw[0]:raw);return Number.isFinite(value)?String(Math.round(value*10)/10):"";}
...
useEffect(()=>{
 ...
 const coreGeoFeatures=entities.filter(e=>e.template==="GEOENTITY"&&!hiddenGeoIds.has(e.entity_id)&&!hiddenGeoClasses.has(geoClass(e))).flatMap(e=>{const raw=e.components?.geometry?.geojson||e.components?.geometry||e.components?.geojson;if(!raw?.type)return[];const s=e.components?.style||{};const cls=geoClass(e);return[{type:"Feature",properties:{entity_id:e.entity_id,name:nameOf(e),class_name:cls,is_sounding:cls==="SOUNDG",sounding_value:soundingValue(e),fillColor:s.fillColor,lineColor:s.lineColor,circleColor:s.circleColor,fillOpacity:s.fillOpacity,lineWidth:s.lineWidth,circleRadius:s.circleRadius},geometry:raw}]});
 ...
 if(!map.getLayer("core-geoentities-soundings"))map.addLayer({id:"core-geoentities-soundings",type:"symbol",source:"core-geoentities",filter:["all",["==",["geometry-type"],"Point"],["==",["get","is_sounding"],true],["!=",["get","sounding_value"],""]],layout:{"text-field":["get","sounding_value"],"text-size":9,"text-allow-overlap":false,"text-ignore-placement":false},paint:{"text-color":"#9bdcf5","text-halo-color":"#071018","text-halo-width":1}});
