...
export default function App(){
 ...
 useEffect(()=>{Promise.all([fetch(`${API}/api/v1/entities`),fetch(`${API}/api/v1/tasks`),fetch(`${API}/api/v1/task-definitions`),fetch(`${API}/api/v1/artifacts`),fetch(`${API}/api/v1/gis/layers`)]).then(async([e,t,d,a,g])=>{if(e.ok){const items:Entity[]=await e.json();setEntities(items);setHistory(p=>items.reduce((acc,item)=>appendLiveSample(acc,item),p));}if(t.ok)setTasks(await t.json());if(d.ok)setDefs(await d.json());if(a.ok)setOverlays((await a.json()).filter((x:any)=>x.artifact_type==="MAP_OVERLAY"));if(g.ok)setGisLayers(await g.json());});},[]);
 ...
 useEffect(()=>{
  const map=mapRef.current;if(!map||!mapReady||!gisLayers.length)return;
  let canceled=false;
  const refresh=async()=>{
   ...
   for(const layer of gisLayers){
    ...
    const sourceId=`gis-source-${key.replace(/[^a-zA-Z0-9_-]/g,"-")}`;
    const baseId=`gis-${key.replace(/[^a-zA-Z0-9_-]/g,"-")}`;
    ...
    try{
     ...
     data.features=(data.features||[]).map((feature:any)=>{const props=feature.properties||{};return {...feature,properties:{...props,__og_plugin_id:layer.plugin_id,__og_layer_id:layer.layer_id,__og_layer_name:layer.name,__og_icon:icon,__og_color:encFeatureColor(props)}}});
     if(existing)existing.setData(data);else map.addSource(sourceId,{type:"geojson",data});
  ...
  return()=>{canceled=true;map.off("moveend",refresh)};
 },[gisLayers,mapReady]);
 ...
 const load=()=>fetch(`${API}/api/v1/entities/${selectedEntity.entity_id}/location-history?limit=5000`)
  .then(r=>r.ok?r.json():[])
...
useEffect(()=>{
 ...
 const move=(ev:any)=>{
  ...
  const layers=["entity-icons",...gisIds].filter(id=>Boolean(map.getLayer(id)));
