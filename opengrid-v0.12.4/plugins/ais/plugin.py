...
async def call(client: httpx.AsyncClient, method: str, path: str, **kwargs):
    return await sdk_client.request(method, path, **kwargs)
    ...
    await call(client, "POST", "/api/v1/plugins/register", json={
        ...
        "configuration_schema": {
            ...
            "properties": {
                ...
                "websocket_api_key": {"type": "string", "title": "AISStream API key", "secret": True, "description": "API key from aisstream.io. Leading/trailing whitespace is removed."},
...
async def websocket_source(config: dict[str, Any]) -> AsyncIterator[dict[str, Any]]:
    url = str(config.get("websocket_url") or "wss://stream.aisstream.io/v0/stream").strip()
    provider = str(config.get("websocket_provider") or "aisstream").lower().strip()
    ...
    try:
        async with websockets.connect(
            ...
            metrics["last_connect_time"] = datetime.now(timezone.utc).isoformat()
            metrics["last_error"] = None
            if provider == "aisstream":
                api_key = str(config.get("websocket_api_key") or "").strip()
                if not api_key:
                    raise RuntimeError("AISStream API key is required")
                boxes = config.get("bounding_boxes") or [[[-90, -180], [90, 180]]]
                subscription = {"APIKey": api_key, "BoundingBoxes": boxes}
                mmsi = [str(x).strip() for x in (config.get("filter_mmsi") or []) if str(x).strip()]
                message_types = [str(x).strip() for x in (config.get("filter_message_types") or []) if str(x).strip()]
                if mmsi:
                    subscription["FiltersShipMMSI"] = mmsi
                if message_types:
                    subscription["FilterMessageTypes"] = message_types
                # AISStream requires the subscription within three seconds of opening.
                await asyncio.wait_for(websocket.send(json.dumps(subscription)), timeout=2.5)
            else:
            ...
            async for raw in websocket:
                ...
                if isinstance(message, dict) and (message.get("error") or message.get("Error")):
                    raise RuntimeError(str(message.get("error") or message.get("Error")))
                if provider == "aisstream":
                    # AISStream has no separate success ACK; the first AIS frame confirms authentication.
        ...
        metrics["connected"] = False
        metrics["authenticated"] = False
        metrics["last_disconnect_time"] = datetime.now(timezone.utc).isoformat()
