...
adapter=OpenGridAdapter(API,AdapterManifest(
 plugin_id=PLUGIN,name="Fixed Camera Adapter",version="0.4.0",plugin_type="SENSOR_ADAPTER",protocol="CAMERA",
...
async def publish(cam,is_live=True):
 ...
 await adapter.publish_entity(cam["id"],{
  ...
  "sensors":{"items":[{
    ...
    "media":{"snapshot_url":cam.get("snapshot_url",""),"stream_url":cam.get("stream_url",""),"stream_type":str(cam.get("stream_type","HLS")).upper(),"live_available":bool(cam.get("snapshot_url") or cam.get("stream_url"))},
