...
def enc_color(properties: dict[str, Any] | None) -> str | None:
    properties = properties or {}
    raw = properties.get("COLOUR", properties.get("Colour", properties.get("colour")))
    if isinstance(raw, list): raw = raw[0] if raw else None
    if isinstance(raw, str): raw = next((x for x in re.split(r"[;, ]+", raw) if x), None)
    try: code = int(float(raw))
    except (TypeError, ValueError): return None
    return {1:"#f7f7f2",2:"#15191d",3:"#e53935",4:"#20a957",5:"#2787d8",6:"#f2cf32",7:"#8d969d",8:"#8b5a2b",9:"#ffbf00",10:"#8b5fc7",11:"#f57c00",12:"#d73ea8",13:"#f48fb1"}.get(code)

def style_for(name: str, geometry_type: str, properties: dict[str, Any] | None = None) -> dict[str, Any]:
    n = name.lower()
    ...
    if any(x in n for x in ("boy", "bcn", "light")):
        return {"circleColor": enc_color(properties) or "#ffd65c", "circleRadius": 5}
...
def entity_from_feature(layer: dict[str, Any], source_id: str, props: dict[str, Any], geometry: dict[str, Any]) -> dict[str, Any]:
    ...
    # OGR commonly preserves sounding depth as the third coordinate instead of
    # duplicating it into VALSOU. Keep it available to the renderer and tasking.
    coords=geometry.get("coordinates") or []
    if class_name.upper().startswith("SOUNDG") and geometry.get("type")=="Point" and len(coords)>=3:
        attributes.setdefault("VALSOU",coords[2])
    components={
        "aliases":{"name":feature_name(class_name,attributes,source_id)},
        "ontology":{
            "template":"GEOENTITY",
            "domain":"MARITIME",
        ...
        "style":style_for(class_name,str(geometry.get("type") or ""),attributes),
...
def expanded_geoentities(layer: dict[str, Any], source_id: str, props: dict[str, Any], geometry: dict[str, Any]):
    """Expand S-57 sounding MultiPoints into normal selectable Point Geo-Entities."""
    class_name=str(layer.get("class_name") or layer.get("name") or "").upper()
    if class_name.startswith("SOUNDG") and geometry.get("type")=="MultiPoint":
        rows=[]
        for idx,coord in enumerate(geometry.get("coordinates") or []):
            if not isinstance(coord,list) or len(coord)<2:
                continue
            point_props=dict(props)
            if len(coord)>=3:
                point_props["VALSOU"]=coord[2]
            rows.append(entity_from_feature(layer,f"{source_id}-{idx}",point_props,{"type":"Point","coordinates":coord}))
        return rows
    return [entity_from_feature(layer,source_id,props,geometry)]
...
async def publish_geoentities(layer: dict[str, Any], collection: dict[str, Any]):
    ...
    for idx,feature in enumerate(collection.get("features") or []):
        ...
        rows.extend(expanded_geoentities(layer,source_id,props,geometry))
    batch_size=200
...
async def discover_local_layers(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for path in geojson_files:
        try:
            ...
            if existing_mtime != source_mtime_ns or not feature_exists or published_marker < 3:
                await cache_features(info, collection)
            info["metadata"]["geoentity_publish_version"] = 3
...
app = FastAPI(title="OpenGrid NOAA ENC Plugin", version="0.13.5", lifespan=lifespan)
