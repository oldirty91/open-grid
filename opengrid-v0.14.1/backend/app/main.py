...
from fastapi import Depends, FastAPI, HTTPException, Query, WebSocket, WebSocketDisconnect, UploadFile, File
...
async def seed_demo_overlay():
    """Seed aerial imagery as the optional visual basemap.

    Nautical information is supplied by the NOAA ENC plugin as vector data;
    this artifact intentionally contains imagery only.
    """
    metadata = {
        "overlay_type": "XYZ",
        ...
        "navigation_warning": "Reference imagery only",
    }
    async with __import__("app.db", fromlist=["engine"]).engine.begin() as conn:
        # Remove the retired raster chart artifact if it was seeded previously.
        await conn.execute(text("DELETE FROM artifacts WHERE object_name='system/noaa-enc-block-island-sound.json'"))
        await conn.execute(
            text("""
            ...
            {
                "name": "Aerial Imagery",
                "object_name": "system/aerial-imagery.json",
...
async def health():
    return {"status": "ok", "service": "opengrid-api", "version": "0.14.1", "artifact_storage_ready": bool(getattr(app.state, "artifact_storage_ready", False))}
...
@app.post("/api/v1/entities/bulk", tags=["Entities"], summary="Bulk create or replace entities")
async def bulk_upsert_entities(bodies: list[EntityUpsert], db: AsyncSession = Depends(get_db)):
    results = []
    for body in bodies:
        ...
        result = await db.execute(text("""
            INSERT INTO entities_current (
              ...
              :entity_id, 1, :is_live, :template, :expiry_time,
              CAST(:components AS JSONB), CAST(:component_provenance AS JSONB), CAST(:component_times AS JSONB),
              CASE WHEN :has_point THEN ST_SetSRID(ST_Point(:lon,:lat),4326) ELSE NULL END
            )
            ON CONFLICT (entity_id) DO UPDATE SET
              revision = entities_current.revision + 1, is_live = EXCLUDED.is_live,
              template = EXCLUDED.template, expiry_time = EXCLUDED.expiry_time,
              components = EXCLUDED.components, component_provenance = EXCLUDED.component_provenance,
              component_times = EXCLUDED.component_times, geom = EXCLUDED.geom, updated_at = NOW()
            RETURNING entity_id, revision, is_live, template, expiry_time, components,
                      component_provenance, component_times, created_at, updated_at
        """), {
            "entity_id": entity_id, "is_live": body.is_live, "template": template,
            "expiry_time": body.expiry_time, "components": json.dumps(body.components),
            "component_provenance": json.dumps(provenance_map),
            "component_times": json.dumps(component_times_for(body)),
            "has_point": point is not None, "lat": point[0] if point else 0, "lon": point[1] if point else 0,
        })
        entity = dict(result.mappings().one())
        await db.execute(text("""
            INSERT INTO entity_revisions(entity_id,revision,operation,payload,provenance)
            VALUES(:entity_id,:revision,'ENTITY_REPLACE',CAST(:payload AS JSONB),CAST(:provenance AS JSONB))
        """), {"entity_id":entity_id,"revision":entity["revision"],"payload":json.dumps(body.components),"provenance":json.dumps(body.provenance)})
...
@app.delete("/api/v1/entities/{entity_id}", tags=["Entities"], summary="Delete one entity")
async def delete_entity(entity_id: str, db: AsyncSession = Depends(get_db)):
    result = await db.execute(text("""
        DELETE FROM entities_current
    ...
    await db.commit()
    await emit("entity.deleted", {"entity_id": entity_id})
    return {"deleted": True, "entity_id": entity_id}


@app.delete("/api/v1/entities/by-source/{source_system}", tags=["Entities"], summary="Delete entities published by a source")
async def delete_entities_by_source(source_system: str, source_dataset: str | None = None, db: AsyncSession = Depends(get_db)):
    if source_dataset:
        result = await db.execute(text("""
            DELETE FROM entities_current
            WHERE components->'provenance'->>'source_system'=:source_system
              AND components->'provenance'->>'source_dataset'=:source_dataset
            RETURNING entity_id
    ...
    for entity_id in ids:
        await emit("entity.deleted", {"entity_id":entity_id})
    return {"deleted":len(ids),"entity_ids":ids}
...
@app.get(
    ...
    tags=["Entities"],
    summary="Read persisted entity location history",
)
async def location_history(
    ...
    """), {"entity_id": entity_id, "limit": limit, "hours": hours})
    return [
        {
...
@app.get(
    "/api/v1/tracks/history",
    tags=["Entities"],
    summary="Read persisted track history for all entities",
    description="Returns a mapping of entity IDs to chronologically ordered persisted location samples.",
)
async def all_track_history(
    hours: int = Query(24, ge=1, le=24 * 30),
    limit_per_entity: int = Query(5000, ge=1, le=50000),
    ...
    result = await db.execute(text("""
        WITH ranked AS (
            SELECT
                s.entity_id, s.revision, s.latitude, s.longitude,
                s.heading_degrees, s.speed_mps, s.observed_at,
                ROW_NUMBER() OVER (
                    PARTITION BY s.entity_id
                    ORDER BY s.observed_at DESC, s.revision DESC
                ) AS rn
            FROM entity_location_samples s
            JOIN entities_current e ON e.entity_id = s.entity_id
            WHERE s.observed_at >= NOW() - make_interval(hours => :hours)
              AND (:live_only = FALSE OR e.is_live = TRUE)
        )
        SELECT entity_id, revision, latitude, longitude,
               heading_degrees, speed_mps, observed_at
        FROM ranked
        WHERE rn <= :limit_per_entity
        ORDER BY entity_id, observed_at ASC, revision ASC
    """), {
        "hours": hours,
    ...
    for row in result.mappings():
        tracks.setdefault(row["entity_id"], []).append({
            "revision": row["revision"],
            "timestamp": row["observed_at"],
            "latitude": row["latitude"],
            "longitude": row["longitude"],
    ...
    return {"hours": hours, "tracks": tracks}
...
@app.put("/api/v1/plugins/{plugin_id}/gis-layers", tags=["Plugins"])
async def register_gis_layers(plugin_id: str, body: PluginGisLayerRegistration, db: AsyncSession = Depends(get_db)):
    if not (await db.execute(text("SELECT 1 FROM plugins WHERE plugin_id=:id"), {"id": plugin_id})).first():
        raise HTTPException(404, "Plugin not registered")
    await db.execute(text("DELETE FROM plugin_gis_layers WHERE plugin_id=:id"), {"id": plugin_id})
    for layer in body.layers:
        await db.execute(text("""
            INSERT INTO plugin_gis_layers(
                plugin_id,layer_id,name,description,geometry_type,service_url,
                default_visible,min_zoom,max_zoom,style,metadata
            ) VALUES(
                :plugin_id,:layer_id,:name,:description,:geometry_type,:service_url,
                :default_visible,:min_zoom,:max_zoom,CAST(:style AS JSONB),CAST(:metadata AS JSONB)
            )
        """), {
            "plugin_id": plugin_id, "layer_id": layer.layer_id, "name": layer.name,
            "description": layer.description, "geometry_type": layer.geometry_type,
            "service_url": layer.service_url, "default_visible": layer.default_visible,
            "min_zoom": layer.min_zoom, "max_zoom": layer.max_zoom,
            "style": json.dumps(layer.style), "metadata": json.dumps(layer.metadata),
        })
    await db.commit()
    return {"plugin_id": plugin_id, "layers_registered": len(body.layers)}

@app.get("/api/v1/gis/layers", tags=["Plugins"])
async def list_gis_layers(db: AsyncSession = Depends(get_db)):
    result = await db.execute(text("""
        SELECT l.*, p.name AS plugin_name, p.enabled, p.status
        FROM plugin_gis_layers l JOIN plugins p ON p.plugin_id=l.plugin_id
        WHERE p.enabled=TRUE ORDER BY p.name,l.name
    """))
    return [dict(row) for row in result.mappings()]

@app.post("/api/v1/gis/layers/{plugin_id}/{layer_id}/hide-class", tags=["Plugins"])
async def hide_gis_layer_class(plugin_id: str, layer_id: str, db: AsyncSession = Depends(get_db)):
    row = (await db.execute(text("""
        SELECT l.name,l.metadata,p.configuration
        FROM plugin_gis_layers l JOIN plugins p ON p.plugin_id=l.plugin_id
        WHERE l.plugin_id=:plugin_id AND l.layer_id=:layer_id
    """), {"plugin_id": plugin_id, "layer_id": layer_id})).mappings().first()
    if not row:
        raise HTTPException(404, "GIS layer not found")
    metadata = dict(row.get("metadata") or {})
    class_name = str(metadata.get("class_name") or row["name"]).strip()
    configuration = dict(row.get("configuration") or {})
    disabled = [str(x) for x in configuration.get("disabled_classes", [])]
    if class_name not in disabled:
        disabled.append(class_name)
    configuration["disabled_classes"] = disabled
    await db.execute(text("UPDATE plugins SET configuration=CAST(:configuration AS JSONB),updated_at=NOW() WHERE plugin_id=:id"), {
        "id": plugin_id, "configuration": json.dumps(configuration),
    })
    await db.execute(text("""
        DELETE FROM plugin_gis_layers
        WHERE plugin_id=:plugin_id AND COALESCE(metadata->>'class_name',name)=:class_name
    """), {"plugin_id": plugin_id, "class_name": class_name})
    await db.commit()
    await hub.broadcast({"type":"plugin.updated","data":{"plugin_id":plugin_id,"configuration":configuration}})
    return {"plugin_id": plugin_id, "hidden_class": class_name, "configuration": configuration}


@app.get("/api/v1/gis/layers/{plugin_id}/{layer_id}/geojson", tags=["Plugins"])
async def proxy_gis_layer(plugin_id: str, layer_id: str, bbox: str = Query(...), db: AsyncSession = Depends(get_db)):
    row = (await db.execute(text("""
        SELECT l.service_url FROM plugin_gis_layers l JOIN plugins p ON p.plugin_id=l.plugin_id
        WHERE l.plugin_id=:plugin_id AND l.layer_id=:layer_id AND p.enabled=TRUE
    """), {"plugin_id": plugin_id, "layer_id": layer_id})).mappings().first()
    if not row:
        raise HTTPException(404, "GIS layer not found")
    import httpx
    try:
        async with httpx.AsyncClient(timeout=45) as client:
            response = await client.get(row["service_url"], params={"bbox": bbox})
            response.raise_for_status()
    except Exception as exc:
        raise HTTPException(502, f"GIS plugin unavailable: {exc}")
    return Response(content=response.content, media_type="application/geo+json")



@app.get("/api/v1/gis/layers/{plugin_id}/{layer_id}/tiles/{z}/{x}/{y}", tags=["Plugins"])
async def proxy_gis_tile(plugin_id: str, layer_id: str, z: int, x: int, y: int, db: AsyncSession = Depends(get_db)):
    row = (await db.execute(text("""
        SELECT l.service_url,l.metadata FROM plugin_gis_layers l JOIN plugins p ON p.plugin_id=l.plugin_id
        WHERE l.plugin_id=:plugin_id AND l.layer_id=:layer_id AND p.enabled=TRUE
    """), {"plugin_id": plugin_id, "layer_id": layer_id})).mappings().first()
    if not row:
        raise HTTPException(404, "GIS layer not found")
    url = str(row["service_url"]).replace("{z}",str(z)).replace("{x}",str(x)).replace("{y}",str(y))
    import httpx
    try:
        ...
        raise HTTPException(502, f"GIS tile provider unavailable: {exc}")
    return Response(content=response.content, media_type=response.headers.get("content-type","application/octet-stream"), headers={"Cache-Control":response.headers.get("cache-control","public,max-age=86400")})
