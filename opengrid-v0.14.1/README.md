...
## v0.13.1 ENC usability and plugin configuration

The retired NOAA raster chart overlay has been replaced by an aerial imagery
overlay. Nautical reference data is now supplied by the data-driven
`plugins/noaa_enc` adapter, which discovers NOAA ENC Direct to GIS layers,
queries only the visible map extent, and caches features in its own PostGIS
schema. The map layer manager exposes imagery and each plugin layer separately.

See `docs/architecture/GIS_LAYERS.md`, `docs/plugins/NOAA_ENC.md`, and
`docs/MAP_LAYERS.md`.
...
## v0.13.2 operational UI cleanup

- Plugins and Artifacts open in separate tabs from the operational map.
- Added searchable Contacts and Assets drawers plus independently collapsible side panels.
- NOAA ENC remains the machine-readable reference-data integration. Local GeoJSON files can be mounted under `plugins/noaa_enc/data` for offline use.


## v0.13.3 Entity categories and local ENC

OpenGrid's world model uses three operational Entity categories:

- **Assets** — systems OpenGrid can control or task.
- **Tracks** — observed contacts from sensors and external feeds; not directly controllable.
- **Geo-Entities** — points, lines, polygons, and ellipses representing geographic operational information.

The NOAA ENC adapter is now local-first. Upload official S-57 `.000` cells from the plugin page; the adapter converts and indexes them locally for offline use. Imported ENC features appear in the Geo-Entities drawer but remain in plugin-owned storage, avoiding pollution of dynamic Entity/Track history tables.


## v0.13.4 Geo-Entity unification

NOAA ENC features now publish through the same core Entity API as every other
Geo-Entity. The map and Geo-Entities drawer no longer rely on a separate plugin
feature cache for ENC objects.


## v0.13.5 map readability

- Assets, Tracks, task routes, and selections remain above ENC Geo-Entities.
- S-57 buoy, beacon, and light colors use the imported `COLOUR` attribute.
- `SOUNDG` Geo-Entities render as small depth values instead of dense white dots.
- Individual Geo-Entities and entire classes can be hidden from the map without removing them from the drawer.


## v0.13.6 ENC presentation fixes

- S-57 `COLOUR` is resolved from each Geo-Entity's attributes at render time.
- Existing ENC cells are republished to replace stale yellow styles.
- `SOUNDG` MultiPoints are expanded into selectable Point Geo-Entities and depth is retained from the Z coordinate.
- Asset and Track hit-testing takes precedence over all Geo-Entities.


## v0.14 core world-model tools

- Operators can create, edit, clone, delete, import, and export Point, LineString,
  Polygon, and Ellipse Geo-Entities.
- Geo-Entities may include optional purpose/tags and an MSL-referenced vertical
  volume using `minimum_elevation_m` and `maximum_elevation_m`.
- Elevation is canonical and always expressed in meters MSL.
- The fixed-camera adapter publishes a flexible `sensors.items` collection and
  no longer publishes singular `sensor` and `video` components.
- Recognized sensor types receive passive map coverage rendering.
- Asset drawer rows show readiness, age, active Task, battery, and sensor count.
- ENC `SOUNDG` entities remain queryable but are hidden from the map by default.


## v0.14.1 Geo-Entity editor and RTSP

- Geo-Entity creation now starts from explicit Point, Line, Polygon, or Ellipse actions.
- Lines and polygons are finished by double-clicking or using the Finish button.
- After drawing, numbered vertices can be dragged directly on the map.
- A coordinate table exposes latitude, longitude, and MSL elevation for every defining point.
- The fixed-camera adapter accepts RTSP sources for FFmpeg snapshots and recordings.
- RTSP is not played directly in the browser because browsers do not support native RTSP playback.
