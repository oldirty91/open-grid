...
DEFAULT_CAMERAS=[{
 ...
 "latitude":41.4302,"longitude":-71.4669,"elevation_m":8,
...
SCHEMA={"type":"object","properties":{"cameras":{"type":"array","items":{"type":"object","required":["id","name","latitude","longitude"],"properties":{
 ...
 "latitude":{"type":"number"},"longitude":{"type":"number"},"elevation_m":{"type":"number"},
 "direction_deg":{"type":"number"},"horizontal_fov_deg":{"type":"number"},"vertical_fov_deg":{"type":"number"},"range_m":{"type":"number"},
 "snapshot_url":{"type":"string"},"stream_url":{"type":"string"},"stream_type":{"type":"string","enum":["HLS","MP4","MJPEG","IMAGE","RTSP"]}
}}}}}

adapter=OpenGridAdapter(API,AdapterManifest(
 plugin_id=PLUGIN,name="Fixed Camera Adapter",version="0.4.1",plugin_type="SENSOR_ADAPTER",protocol="CAMERA",
...
async def ffmpeg_capture(url:str, duration:int|None=None)->tuple[bytes,str,str]:
 ...
 with tempfile.TemporaryDirectory() as td:
  ...
  input_args=["-rtsp_transport","tcp"] if url.lower().startswith("rtsp://") else []
  if duration:
   cmd=["ffmpeg","-y","-loglevel","error",*input_args,"-i",url,"-t",str(duration),"-c:v","libx264","-preset","veryfast","-an","-movflags","+faststart",str(out)]
  else:
   cmd=["ffmpeg","-y","-loglevel","error",*input_args,"-i",url,"-frames:v","1",str(out)]
...
async def publish(cam,is_live=True):
 ...
 await adapter.publish_entity(cam["id"],{
  ...
  "location":{"latitude":float(cam["latitude"]),"longitude":float(cam["longitude"]),"elevation_m":float(cam.get("elevation_m",cam.get("altitude_m",0))),"heading_degrees":float(cam.get("direction_deg",0)),"speed_mps":0},
  "status":{"state":"ONLINE" if is_live else "DISABLED"},
  "sensors":{"items":[{
    "sensor_id":f"{cam['id']}-main-camera",
    "type":"VIDEO_CAMERA",
    "name":cam.get("sensor_name","Main Camera"),
    "status":{"state":"ONLINE" if is_live else "DISABLED"},
    "mount":{"azimuth_deg":float(cam.get("direction_deg",0)),"elevation_deg":float(cam.get("sensor_elevation_deg",0))},
    "field_of_view":{"horizontal_deg":float(cam.get("horizontal_fov_deg",60)),"vertical_deg":float(cam.get("vertical_fov_deg",35)),"range_m":float(cam.get("range_m",500))},
    "media":{"snapshot_url":cam.get("snapshot_url",""),"stream_url":cam.get("stream_url",""),"stream_type":str(cam.get("stream_type","HLS")).upper(),"live_available":bool(cam.get("snapshot_url") or cam.get("stream_url")),"browser_live_available":str(cam.get("stream_type","HLS")).upper()!="RTSP"},
