...
- Added explicit Point, Line, Polygon, and Ellipse creation actions.
- Added double-click completion for multi-point geometry.
- Added draggable numbered vertex handles after drawing.
- Added an editable latitude/longitude/elevation coordinate table.
- Added RTSP camera source support for snapshots and recordings through FFmpeg.


## v0.14.0

- Added operator-created Point, LineString, Polygon, and Ellipse Geo-Entities.
- Added create/edit/clone/delete, drawing undo/redo, and GeoJSON import/export.
- Added optional MSL elevation volumes for Geo-Entities.
- Migrated the fixed-camera adapter to `sensors.items`.
- Added generic passive sensor-coverage rendering.
- Added compact readiness, Task, battery, sensor-count, and age data to Asset rows.
- Hid ENC soundings from map presentation by default while retaining the data.


## v0.13.6

- Fixed ENC buoy/beacon/light coloring by reading `COLOUR` directly from Geo-Entity attributes.
- Forced existing ENC imports to republish with current presentation metadata.
- Expanded S-57 `SOUNDG` MultiPoints into individual depth Geo-Entities.
- Rendered sounding depth values from `VALSOU` or geometry Z values.
- Prioritized Asset and Track clicks over ENC Geo-Entities.


## v0.13.5

- Promoted Assets, Tracks, Tasks, and selections above ENC rendering.
- Restored ENC color presentation from S-57 `COLOUR`.
- Rendered `SOUNDG` features as small depth labels instead of dots.
- Added persistent Hide/Show Entity and Hide/Show Class controls.
- Hidden Geo-Entities remain searchable in the Geo-Entities drawer.


## v0.13.4

- Unified NOAA ENC objects with the core GEOENTITY model.
- Added bulk Entity publication for data adapters.
- Removed the parallel ENC map-only feature path.
- ENC points, lines, and polygons now appear in the standard Geo-Entities drawer and inspector.


## v0.13.3

- Added Assets, Tracks, and Geo-Entities as the three operator-facing Entity categories.
- Added a dedicated Geo-Entities drawer for core GEOENTITY records and plugin GIS features.
- Removed stale registry records from the retired offline-map experiment during API startup.
- Reworked NOAA ENC as a local-first S-57 importer.
- Added `.000-.999` ENC upload, conversion, dataset listing, and removal in the plugin page.
- Added GDAL-based S-57 conversion and offline PostGIS indexing.
- Updated Entity and ENC documentation.


## v0.13.2

- Added Contacts and Assets header drawers
- Added independent left/right panel collapsing
- Plugins and Artifacts now open in new tabs
- Added local ENC data mount groundwork


## 0.13.1
- Added bulk database-backed track history hydration.
- Kept NOAA ENC registered through the unified GIS API during the refactor.

# 0.13.1

- Added local GIS tile proxying with cache headers.
- Restored 24-hour persisted tracks on initial map load for all entities.
- Live samples are merged with database history.
...
- Added automatic NOAA ENC scale selection across overview, general, coastal, approach, and harbour services.
- Added zoom-aware layer ranges so the most appropriate ENC usage is displayed automatically.
- Reworked map loading to merge viewport results into a persistent client cache instead of replacing visible features.
- Added padded viewport prefetching, debounced refreshes, and stale-request protection to reduce feature flicker while zooming and panning.
- Added a **Hide this class** action to the ENC feature inspector. Hidden classes apply across every ENC scale and remain editable in plugin configuration.
- Added map loading status and immediate cleanup of classes hidden from the inspector.

## v0.12.3

- Restored the schema-driven MAVLink configuration form.
- Fixed primitive array fields so ENC discovery filters can be edited and cleared.
- Hardened AISStream configuration, connection diagnostics, authentication detection, and reconnect behavior.
- Added S-57 `COLOUR` presentation mapping for ENC point symbols, including red (3) and green (4) buoys.


- Restored schema-driven configuration forms for AIS, ADS-B, MAVLink, NOAA ENC, and future plugins.
- Kept raw JSON configuration under an Advanced view.
- Removed the redundant floating Map Layers panel.
- Aerial imagery remains controlled from the Aerial Imagery Artifact.
- NOAA ENC publishes every discovered supported layer by default.
- Added NOAA ENC per-layer visibility controls to plugin configuration.
- Added selectable ENC features with a right-side feature inspector.
- Added simplified nautical symbology for buoys, beacons, lights, wrecks, obstructions, and soundings.
- Preserved native line and polygon rendering for ENC geometry.

# v0.12.1

- Replaced the non-working NOAA raster chart overlay with aerial imagery.
- Added the NOAA ENC GIS plugin with layer discovery, bounded GeoJSON queries,
  plugin-owned PostGIS storage, and stale-cache fallback.
- Added generic GIS layer registration, discovery, and proxy endpoints.
- Added a map layer manager for imagery and plugin data.
- Added GIS and NOAA ENC developer documentation.
...
## 0.11.4
- Removed the obsolete RIDOT camera registry entry.
- Added camera CRUD configuration to the Plugins page with live reload.
- Added an Apple public HLS test stream and hls.js playback.
- Added real FFmpeg-backed snapshots and recordings from configured streams.
- Changed NOAA NCDS to an opaque chart basemap layer to prevent basemap/ENC competition and restore water portrayal.

# OpenGrid v0.11.4

## Fixed

- Added retry-safe API database initialization for fresh PostgreSQL/PostGIS volumes.
- Prevented the API from failing when the PostGIS entrypoint briefly restarts PostgreSQL after bootstrap.
- Changed simulator and adapter startup dependencies to wait for API health.
