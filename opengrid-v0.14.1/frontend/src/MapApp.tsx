...
function LiveCamera({video}:{video:any}){
 const type=String(video?.stream_type||"").toUpperCase();
 if(type==="RTSP")return <div className="rtspNotice"><strong>RTSP source configured</strong><small>Browsers cannot play RTSP directly. Snapshot and recording Tasks use FFmpeg through the camera adapter.</small></div>;
 if(type==="IMAGE"||type==="MJPEG")return <img className="liveCamera" src={video.snapshot_url||video.stream_url}/>;
...
export default function App(){
 ...
 const geoVertexMarkersRef=useRef<maplibregl.Marker[]>([]);
 ...
 useEffect(()=>{
  ...
  geoVertexMarkersRef.current.forEach(marker=>marker.remove());
  geoVertexMarkersRef.current=[];
  if(!map||!geoDraft||!geoReview)return;
  geoDraft.points.forEach((point,index)=>{
   const element=document.createElement("div");
   element.className="geoVertexMarker";
   ...
   marker.on("dragend",()=>{
    ...
    updateGeoPoint(index,{lon:Number(lngLat.lng.toFixed(7)),lat:Number(lngLat.lat.toFixed(7))});
   });
   geoVertexMarkersRef.current.push(marker);
  });
  return()=>{geoVertexMarkersRef.current.forEach(marker=>marker.remove());geoVertexMarkersRef.current=[]};
 },[geoDraft?.points.length,geoDraft?.geometryType,geoReview]);
...
});map.on("dblclick",(ev:MapMouseEvent)=>{
 const draft=geoDraftRef.current;
 if(!draft)return;
 ev.preventDefault();
 if((draft.geometryType==="LineString"&&draft.points.length>=2)||(draft.geometryType==="Polygon"&&draft.points.length>=3))setGeoReview(true);
...
function updateGeoPoint(index:number,patch:Partial<DrawPoint>){setGeoDraft(d=>d?{...d,points:d.points.map((p,i)=>i===index?{...p,...patch}:p)}:d)}
 function removeGeoPoint(index:number){setGeoDraft(d=>d?{...d,points:d.points.filter((_,i)=>i!==index)}:d)}
